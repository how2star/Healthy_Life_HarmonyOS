...
/**
 * 卡片管理的工具
 * 用于管理 HarmonyOS 应用中的 ** 卡片（Form）** 功能，包括卡片数据的增删改查、定时更新以及与数据库的交互。
 * 该类封装了卡片管理的核心逻辑，并通过单例模式提供全局访问。
 */
class FormUtils {
    /**
     * Insert form data
        ...
        data.forEach((item: TaskInfo) => {
            ...
            switch (item.taskID) {
                ...
                case taskType.sleepEarly:
                    ...
                    break;
                case taskType.run:
                    temp = this.getTemp(temp, TaskType.Run, Unit.Kilometer, false, item.isDone);
                    break;
                default:
                    break;
            }
